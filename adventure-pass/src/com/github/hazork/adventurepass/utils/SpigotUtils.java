...
public class SpigotUtils {
  ...
  public static Player getPlayer(Object object) {
    if (isPlayer(object)) return (Player) object; else return null;
  }
  ...
  public static void sendSoundAll(Sound sound) {
    Bukkit
      .getOnlinePlayers()
      .stream()
      .forEach(p -> p.playSound(p.getLocation(), sound, 3.0F, 0.5F));
  }
  ...
  public static ItemStack fromMaterial(
    Material material,
    String displayName,
    String... lorelines
  ) {
  ...
  public static Material getRandomMaterial(Predicate<Material> cond) {
    ...
    while (!cond.test(request)) request = getRandomMaterial();
    return request;
  }
