...
public class PassMenu extends Menu {
  ...
  public static Function<Integer, ItemStack> EMPTY = i ->
  ...
  public static Function<Integer, ItemStack> EMPTY_PREMIUM = i ->
    SpigotUtils.fromMaterial(
      Material.YELLOW_STAINED_GLASS_PANE,
      "§bNível §ePremium§b: §f" + i,
      "§cNível sem prêmio.",
      "",
    ...
    for (int i = 0; i < 9; i++) {
      ...
      if (level.equals(getPassHandler().getLevel())) {
        inventory.setItem(
          9 + i,
          SpigotUtils.setMeta(
            SpigotUtils.getHead("MHF_ArrowDown"),
            false,
            "§aEste é o seu nível atual",
            "§7Progresso:",
            getPassHandler().getProgressBar()
          )
        );
  ...
  private ItemStack levelToItem(Level level) {
    return level == null
      ? EMPTY.apply(Integer.MIN_VALUE)
      : level.getPrize() == null
        ? EMPTY.apply(level.getNumber())
        : SpigotUtils.fromMaterial(
          getPassHandler().getIntLevel() >= level.getNumber(),
          level.getMaterial(),
          "§bNível: §f" + level.getNumber(),
