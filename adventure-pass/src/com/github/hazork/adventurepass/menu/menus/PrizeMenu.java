...
public class PrizeMenu extends Menu {
  ...
  private static UnaryOperator<Integer> slotFunction = i ->
    (i == 17 || i == 26) ? i + 2 : i;
  private static final ItemStack EMPTY_ITEM = SpigotUtils.fromMaterial(
    false,
    Material.GLASS_PANE,
    "§cSlot vazio",
    "§7Sem mais itens para",
    "§7coletar."
  );
    ...
    if ((page + 1) < prizeList.size()) {
      inventory.setItem(
        ...
        SpigotUtils.fromMaterial(
          Material.LIME_DYE,
          "§aAvançar",
          "§7Veja os prêmios disponíveis da próxima página.",
          "",
    ...
    if (page > 0) {
      inventory.setItem(
        ...
        SpigotUtils.fromMaterial(
          Material.RED_DYE,
          "§cVoltar",
          "§7Veja os prêmios disponíveis da página anterior.",
          "",
          "§cClique para voltar"
        )
      );
  ...
  private ItemStack prizeToItem(Prize prize) {
    return SpigotUtils.fromMaterial(
      Material.CHEST,
      "§a" + prize.getName(),
      "§fPrêmio: §7" + prize.getDescription(),
      "",
